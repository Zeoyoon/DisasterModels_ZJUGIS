...
class RoadLoss2(nn.Module):
    ...
    def forward(self, score, target):
        ...
        roadspeed_loss = self._forward2(score[2], target2)
        road_loss = self._forward2(score[2], target2)
...
class RoadLossBinary(nn.Module):
    def __init__(self):
        super(RoadLossBinary, self).__init__()
        
        self.bce_loss = nn.BCEWithLogitsLoss()
        self.ce_loss = nn.CrossEntropyLoss()
        self.rmi_loss = RMILoss(num_classes=3)
    ...
    def forward(self, score, target):
        ...
        # we want two types of targets, target 1 (B, H, W ) : C (0-7), 0~6 road speed, 7: no road (background)
        # target 2 (B, 1, H, W) : 0 no road, 1 road
        target = target.float()   # b, 1, h, w
        
        roadrmi_loss = self._forward1(score, target)   # 一个基于rmi
        road_loss = self._forward2(score, target)  # 一个基于bce
        total_loss = road_loss + 0.8 * roadrmi_loss
