...
class Unetupup(nn.Module):
    def __init__(self, num_classes,deep_supervision=False,att_phi = 2):
        super(Unetupup, self).__init__()
        self.att_phi = att_phi
        if att_phi >= 1 and att_phi <= 3:  # 注意力机制的设置
            self.attention00 = attention_block[att_phi -1](64)
            self.attention0 = attention_block[att_phi - 1](128)
            self.attention1 = attention_block[att_phi - 1](256)
            self.attention2 = attention_block[att_phi - 1](512)
            self.attention3 = attention_block[att_phi - 1](1024)
    ...
    def forward(self, x):
        #添加注意力机制部分
        x_0_0 = self.attention00(self.stage_0(x))
        x_1_0 = self.attention0(self.stage_1(self.pool(x_0_0)))
        x_2_0 = self.attention1(self.stage_2(self.pool(x_1_0)))
        x_3_0 = self.attention2(self.stage_3(self.pool(x_2_0)))
        x_4_0 = self.attention3(self.stage_4(self.pool(x_3_0)))
        #不添加注意力机制部分
