...
if __name__ == "__main__":
    ...
    name_classes = ["_background_", "fire"]
    # -------------------------------------------------------#
    #   指向VOC数据集所在的文件夹
    #   默认指向根目录下的VOC数据集
    # -------------------------------------------------------#
    VOCdevkit_path = '/exstorage/xyc/Mask_RCNN-master/unet-pytorch-main/VOCdevkit'

    image_ids = open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Segmentation/test.txt"), 'r').read().splitlines()
    gt_dir = os.path.join(VOCdevkit_path, "VOC2007/SegmentationClass/")
    miou_out_path = "/exstorage/xyc/Mask_RCNN-master/unet-pytorch-main/miou_out"
    ...
    if miou_mode == 0 or miou_mode == 2:
        ...
        print("Get miou done.")
        # with open(os.path.join(miou_out_path, "IoUs.txt"), 'a') as f1:
        #     f1.write(str(IoUs))
        #     f1.write("\n")
        # with open(os.path.join(miou_out_path, "PA_Recall.txt"), 'a') as f2:
        #     f2.write(str(PA_Recall))
        #     f2.write("\n")
        # with open(os.path.join(miou_out_path, "Precision.txt"), 'a') as f3:
        #     f3.write(str(Precision))
        #     f3.write("\n")
