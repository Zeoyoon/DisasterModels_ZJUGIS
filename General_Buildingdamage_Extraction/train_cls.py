...
train_dirs = ['data/samplelib']

models_folder = 'weights'

loc_folder = 'data/samplelib/loc'
...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(try_cnt):
            ...
            _sc = msk2[y0:y0+crop_size, x0:x0+crop_size].sum() * 4 + msk3[y0:y0+crop_size, x0:x0+crop_size].sum() * 5 + msk4[y0:y0+crop_size, x0:x0+crop_size].sum() * 2 #+ msk1[y0:y0+crop_size, x0:x0+crop_size].sum()
            if _sc > bst_sc:
                bst_sc = _sc
                bst_x0 = x0
                bst_y0 = y0
...
if __name__ == '__main__':
    ...
    #scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[1, 3, 5, 11, 17, 23, 29, 33, 47, 50, 60, 70, 90, 110, 130, 150, 170, 180, 190], gamma=0.5)
    scheduler = lr_scheduler.StepLR(optimizer, step_size=3, gamma=0.5)
    
    model = nn.DataParallel(model).cuda()
    
    #snap_to_load = 'res50_loc_{}_0_attr_best'.format(seed)
    ...
    for epoch in range(14):
        ...
        for i, sample in enumerate(iterator):
            ...
            iterator.set_description(
                "epoch: {}; lr {:.7f}; Loss {loss.val:.4f} ({loss.avg:.4f}); cce_loss {loss1.val:.4f} ({loss1.avg:.4f}); Dice {dice.val:.4f} ({dice.avg:.4f})".format(
                    epoch, scheduler.get_lr()[-1], loss=losses, loss1=losses1, dice=dices))
