...
train_dirs = ['data/samplelib_1024']
...
bad_case_file = 'data/bad_case_list_all.csv'
bad_case_list = pd.read_csv(bad_case_file)

bad_case_list = bad_case_list['file_name'].tolist() 
...
for d in train_dirs:
    for f in sorted(listdir(path.join(d, 'images'))):
        if '_pre_' in f and f.replace('_pre_','_post_') not in bad_case_list:
...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if random.random() > 0.985:
            img = cv2.imread(fn.replace('_pre_', '_post_'), cv2.IMREAD_COLOR)
...
if __name__ == '__main__':
    ...
    snapshot_name = 'res50_loc_{}_0_attr'.format(seed)
    ...
    # 0.00015 0.0000187 0.0000012
    model, optimizer = amp.initialize(model, optimizer, opt_level="O1")

    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[1, 5, 15, 29, 43, 53, 65, 80, 90, 100, 110, 130, 150, 170, 180, 190], gamma=0.5)
