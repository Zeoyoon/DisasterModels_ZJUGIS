...
def CIOU_xywh_torch(boxes1,boxes2):
    ...
    center_w = torch.max(boxes1[..., 2],boxes2[..., 2]) - torch.min(boxes1[..., 0],boxes2[..., 0]) #x方向距离
    center_h = torch.max(boxes1[..., 3], boxes2[..., 3]) - torch.min(boxes1[..., 1], boxes2[..., 1]) #y方向距离
    center_w_s = torch.pow(center_w,2) #x方向距离平方
    center_h_s = torch.pow(center_h,2) #y方向距离平方
    center_dis_w_s = torch.pow(((boxes2[..., 2] +  boxes2[...,0])-(boxes1[..., 2] +  boxes1[...,0])), 2) #x方向中心距离平方
    center_dis_h_s = torch.pow(((boxes2[..., 3] + boxes2[..., 1]) - (boxes1[..., 3] + boxes1[..., 1])), 2) #y方向中心距离平方
    eious = ious - (center_dis / outer_diagonal_line+center_dis_w_s/center_w_s+center_dis_h_s/center_h_s)
    return ious,cious
